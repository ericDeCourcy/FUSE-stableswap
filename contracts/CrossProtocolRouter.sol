...
/** @dev This file tries to stay as similar to the deployed fuseswap router as possible, while integrating 
...
// TODO: check that this matches interface imported with ISwapFlashLoanV3 before deleting. If mismatch, investigate issues which may arise
/*
library SafeMath {
...
library CrossProtocolSwapLibrary {  //Note: changed from UniswapV2Library to this
    using SafeMath for uint;

    
    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'CrossProtocolSwapLibrary: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'CrossProtocolSwapLibrary: ZERO_ADDRESS');
    }
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        require(amountA > 0, 'CrossProtocolSwapLibrary: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'CrossProtocolSwapLibrary: INSUFFICIENT_LIQUIDITY');
        amountB = amountA.mul(reserveB) / reserveA;
    }

    // given an input amount of an asset and pair reserves, returns the maximum output amount of the other asset
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
        require(amountIn > 0, 'CrossProtocolSwapLibrary: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'CrossProtocolSwapLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) internal pure returns (uint amountIn) {
        require(amountOut > 0, 'CrossProtocolSwapLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'CrossProtocolSwapLibrary: INSUFFICIENT_LIQUIDITY');
...
abstract contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = CrossProtocolSwapLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = CrossProtocolSwapLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'UniswapV2Router: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = CrossProtocolSwapLibrary.quote(amountBDesired, reserveB, reserveA);
    ...
    function addLiquidity(
        ...
        address pair = CrossProtocolSwapLibrary.pairFor(factory, tokenA, tokenB);
    ...
    function addLiquidityETH(
        ...
        address pair = CrossProtocolSwapLibrary.pairFor(factory, token, WETH);
    ...
    function removeLiquidity(
        ...
        (address token0,) = CrossProtocolSwapLibrary.sortTokens(tokenA, tokenB);
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts = CrossProtocolSwapLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(CrossProtocolSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
    ...
    {
        require(path[path.length - 1] == WETH, 'UniswapV2Router: INVALID_PATH');
        amounts = CrossProtocolSwapLibrary.getAmountsOut(factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, CrossProtocolSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    */
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = CrossProtocolSwapLibrary.sortTokens(input, output);
            IUniswapV2Pair pair = IUniswapV2Pair(CrossProtocolSwapLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            amountOutput = CrossProtocolSwapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, CrossProtocolSwapLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(CrossProtocolSwapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) public pure virtual override returns (uint amountB) {
        return CrossProtocolSwapLibrary.quote(amountA, reserveA, reserveB);
    }
    ...
    {
        return CrossProtocolSwapLibrary.getAmountOut(amountIn, reserveIn, reserveOut);
    }
