// @dev deploys the "deployerV3" contract and a pool instance

const { ethers } = require("hardhat");

async function main() {

    const timeout = 25000; //change to 1000 for local deployments, keep as 25k for fuse network deloyments
    /*console.log("!!! NOTE !!!");
    console.log("Timeout should be 25 seconds for fusenet deployment");
    console.log("!!! NOTE !!!");
    console.log("");*/
    ...
    */
    const INIT = await deployer.deploy(swapflashloan.address, [fakedai.address, fakeusdc.address, fakeusdt.address], [18, 6, 6], "FSS-FAKE-DAI-USDC-USDT-V3", "LP-FAKE-USD-V3", 100, 100, 100, lptoken.address, lprewards.address);
    console.log("pool initialized in hash:", INIT);
  
    await new Promise(r => setTimeout(r, timeout));
  
/*
    // TODO: change all calls to call the DEPLOYED instance instead of the template instance
